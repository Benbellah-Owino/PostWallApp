<script>
    ...
    import MyFollowed from "../../myComponents/myFollowers.svelte"
...
<CreatePostButton/>
{#if menuActive==true} 
<MainNav class="flex flex-col items-center justify-center w-10 float-left z-10" on:dock={dock} />
{:else if menuActive==false}
<button class="main_nav_btn w-12 h-12 border-2 border-amber-400 rounded-full bg-amber-400 text-zinc-900 z-10 fixed" on:click={toggleMenu}><i class="fa-solid fa-bars text-zinc-900 text-sm "></i></button>
{/if} 
